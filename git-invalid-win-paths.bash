...
EOF
	...
	while IFS=	read -r -d $'\0'; do
		matched_paths+=("$REPLY")
	done < <(git -C "$repo_dir" ls-tree --full-name --name-only -r -z "$tree" "${repo_paths[@]:-}")

	log_info "checking ${#matched_paths[@]} paths..."
	invalid_paths=()
	for matched_path in "${matched_paths[@]}"; do
		if [ "${#matched_path}" -gt "260" ]; then
			log_warn "path exceeds max length of 260 chararcters: ${matched_path}"
			invalid_paths+=("${matched_path}")
		elif basename "${matched_path}" | grep -Ei '(^(PRN|AUX|CLOCK\$|NUL|CON|COM[0-9]|LPT[0-9])(\..+)?$|\.{2,}|[[:cntrl:]\\?*:\";|\/<>]+|[[:space:].]$)' 1>/dev/null; then
			log_warn "path name is invalid or contains illegal characters: ${matched_path}"
			invalid_paths+=("${matched_path}")
		fi
	done

	if [ "${#invalid_paths[@]}" -gt "0" ]; then
		log_warn "repository at ${repository} contains invalid Windows/FAT file names:"
		if [[ -z "$null_sep" ]]; then
			printf '%q\n' "${invalid_paths[@]}"
		else
			printf '%s\0' "${invalid_paths[@]}"
		fi
		return 1
	fi
}
